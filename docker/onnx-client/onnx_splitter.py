...
def onnx_model_split_all(onnx_file):
  ...
  for layer in enumerate_model_node_outputs(full_model_onnx):
    #Ignore the output layers and Activation Layers (RELU variations, Tanh, Sigmoid, ecc..)
    if layer not in outputsList and nodeIsNotAnActivation(layer):
...
def nodeIsNotAnActivation(node):
  '''
  Ignore Activation Nodes (RELU variations, Tanh, Sigmoid, ecc..)
  :param node: node to check
  '''
  if ("relu" not in node.lower() and
     "tanh" not in node.lower() and
     "sigmoid" not in node.lower() and
     "affine" not in node.lower() and
     "elu" not in node.lower() and
     "softsign" not in node.lower() and
     "softplus" not in node.lower()):
    return True
  else:
    return False
